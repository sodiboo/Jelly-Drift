...
public class FinishController : MonoBehaviour
{
    ...
    private void Awake() => FinishController.Instance = this;
    ...
    public void Open(bool victory)
    {
        ...
        if (GameState.Instance.gamemode == Gamemode.TimeTrial)
        {
            progressTime.SetProgress(SaveManager.Instance.state.xp, SaveManager.Instance.state.xp + num4, SaveManager.Instance.state.GetLevel(), SaveManager.Instance.state.money, SaveManager.Instance.state.money + num5);
        }
        else
        {
            progressRace.SetProgress(SaveManager.Instance.state.xp, SaveManager.Instance.state.xp + num4, SaveManager.Instance.state.GetLevel(), SaveManager.Instance.state.money, SaveManager.Instance.state.money + num5);
        }
    ...
    private void CheckUnlocks(bool victory)
    {
        ...
        if (GameState.Instance.gamemode != Gamemode.TimeTrial && victory)
        {
            ...
            if (GameState.Instance.difficulty >= DifficultyCycle.Difficulty.Hard && !SaveManager.Instance.state.skins[GameState.Instance.map][1])
            {
                ...
                SaveManager.Instance.Save();
                unlockManager.unlocks.Add(new UnlockManager.Unlock(UnlockManager.UnlockType.Skin, GameState.Instance.map, 1));
            }
        }
