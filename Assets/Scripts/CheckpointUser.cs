...
public class CheckpointUser : MonoBehaviour
{
    ...
    private void Awake() => checkedPoints = new bool[GameController.Instance.checkPoints.childCount];
    ...
    public bool CheckPoint(CheckPoint p)
    {
        ...
        if (ReadyToFinish())
        {
            GameController.Instance.FinishRace(player, base.transform);
            ClearCheckpoint(p.nr);
            return true;
        }
        return false;
    ...
    private void ClearCheckpoint(int n)
    {
        ...
        var component = UnityEngine.Object.Instantiate<GameObject>(PrefabManager.Instance.splitUi).GetComponent<SplitUi>();
    ...
    public void ForceCheckpoint(int n) => checkedPoints[n] = true;
